...
class WpakWebServices {
	...
	public static function build_result_info( $status = 1, $message = '', $app_id = '', $service_slug = '' ) {
	...
	private static function exit_sending_error( $error, $app_id = '', $service = array(), $type = 'json' ) {
	...
	private static function build_final_answer( $service_answer, $app_id, $service_slug ) {
		$final_answer = null;
	...
	private static function exit_sending_web_service_content( $web_service_content, $type = 'json' ) {
		header( 'Content-type: application/json' );
		header( 'Access-Control-Allow-Origin: *' );
		$callback = !empty( $_GET['callback'] ) ? $_GET['callback'] : '';
		if ( $callback ) {
