  ...
  <div class="page-template-template-resource-list">
          ...
          <div class="resource-title-wrapper">
            <div class="clearfix">
              <table cellpadding="0" cellspacing="0" border="0" width="90%">
                <tr>
                  <td class="title"><div class="resource-title-div">
                      <div class="resource-title"> <?php echo showListTitle($resource_id, $resource_type, $resource_popup_image['url'], $resource_popup_url, $resource_slug);?> </div>
                      <!--<div class="resource-type"><?=$resource_type?></div>--> 
                    </div></td>
                  <td class="audio"><?php echo get_audio_preview(get_field('downloads', $resource_id));?></td>
                </tr>
              </table>
              <?php if(!empty($note)){ ?>
              <div class="resource-note"> <a class="icon-note" data-fancybox data-src="#<?=$resource_slug.'-note'?>" href="javascript:;"></a>
                <div id="<?=$resource_slug.'-note'?>" class="hidden-content fancybox-content file-download">
                  <?=$note?>
                </div>
              </div>
              <?php } ?>
    ...
    <div class="resource-footer">
      ...
      <div class="pagination clearfix">
        ...
        $previousLink = get_previous_posts_link( '<button class="btn_gopage_prev"></button>', $total_pages );
        $nextLink = get_next_posts_link( '<button class="btn_gopage_next"></button>' );
        ...
        if ( $wp_query->max_num_pages > 0 ) {
          ...
          for ( $i = 1; $i <= $wp_query->max_num_pages; $i++ ) {
            ...
            echo '<option value="' . $finalurl . 'page/' . $i . '" ' . $selected . '>' . $i . '</option>';
          }
          echo '</select>';
          echo '<span>/' . $wp_query->max_num_pages . '</span>';
        }
