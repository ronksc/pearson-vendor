...
$(document).ready(function(){
	...
	if(isIPAD || ['Macintosh', 'MacIntel', 'MacPPC', 'Mac68K'].indexOf(platform) !== -1) {
		$('#waterLev').css("font-size",'16px');
	}
});
...
var activateFunctions = function() {
	...
	var resetActivity = function() {
		...
		$("#myModal, #blackScreen").fadeOut(function(){
...
var setUserMediaVariable =  function(){
  ...
  if (navigator.mediaDevices.getUserMedia === undefined) {
	...
	var showInstruction = function() {
		...
		if(streamStatus) {
			$('#info-text').html('嘗試讀取水的體積。你可調節平板電腦的方位來使量筒保持直立。同時，攝錄鏡頭會偵測你的眼睛位置。嘗試作答看看你的答案是否正確。');
			$('#videoPopUp').show();
			$('#videoPopUp')[0].load();
			$('#videoPopUp')[0].play();
			...
			$('#info-text').html('上下拖移眼睛來調節視線，從而讀取水的體積。你可嘗試輸入答案，檢查是否正確。');
			$("#demoVid").hide();
			$('#videoPopUp').hide();
			$('#videoPopUp')[0].pause();
	...
	var eyeValue = function() {
		...
		if( ( minLimit ) && ( maxLimit )  ){
			...
			//if($('#mode_1').hasClass("virtualScale")){
				...
				$("#myModal .modal-body p").text('視線已和水位成一直線。').css('line-height','95px');
	...
	var inputProcess = function(getVal) {
		...
		if(currAns == getVal.val()) {
			...
			if(streamStatus) {
				$("#myModal .modal-body p").html('回答正確！<br/>做得好！<input type="button" id=tryButton tabindex="0" value="Ok" style="display:none;">').css('line-height','');
			...
			console.log('wrong');
			$("#myModal .modal-body p").html('回答不正確！<br/><input type="button" id=tryButton tabindex="0" value="再次嘗試">').css('line-height','60px');
			//$("#myModal .modal-body p").html('回答不正確！<br/><div id=tryButton tabindex="0">再次嘗試</div>').css('line-height','60px');
			$('#tryButton').focus();
		}
		$('.closeMod, #tryButton').off("click").on("click",toggleModals);
	}
